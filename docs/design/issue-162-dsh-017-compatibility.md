...
| 预设与 Skill root | `dsh-agent-presets` 已删除；`agent-preset-registry` 暴露 `standingMountFor`、`composedPreset`、`readDocument`、`acquireScope`，内置 `standard` 由 bundle 声明 | live Agent 从确切 mounted generation 和其中唯一 filesystem Skill fiber 读取配置。冷会话只在官方 `standard` 声明的内容摘要、启用状态及当前 root 解析均可证明时恢复；不能证明则停止学习或发布，不猜测写入位置。作用域 lease 必须释放。 |
...
预设的冷会话核验不能仅依靠名称 `standard`：该名称可由 Profile 覆盖。设计要求从 rc.2 官方 bundle 得到可复现的声明摘要，验证运行时 `readDocument` 的规范化输出与启用配置；摘要和来源证据写入探针，不能把某个手工猜出的字符串当作安全证明。live Agent 也必须从其实际绑定的 generation 读取，而非当前默认 preset。

## 实施顺序与验收

1. 针对上述断点先写行为测试：版本门、设置读写/启用唤醒、live 与冷会话 preset 绑定、修改过的 `standard`、handle 关闭、V4 事件、远端未认证请求。然后做最小宿主适配。
