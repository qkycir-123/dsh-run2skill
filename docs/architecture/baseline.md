...
状态：`0.2.0` 核心架构已接受并落地；`0.4.0` 是当前 npm 稳定兼容线，`0.5.0-alpha.1` 为未发布源码候选
文档版本：v0.2
更新时间：2026-09-08
产品输入：docs/product/prd.md v0.2
原始设计证据 baseline：`99f6f02fecdb7dff40c3fbc9470f5907c29f74ca`（`0.1.0-rc.7`）
未发布源码候选 `0.5.0-alpha.1` 的兼容性 baseline：`82a5fd61a7cf5c293cec4bdff68f455398d685e9`（`0.1.3-alpha.2`）；当前稳定版 `0.4.0` 继续绑定 `a66e4702047846cdaa10c66c9d3df3951f5ea70d`（`0.1.2-rc.1`），旧版 `0.3.1` 继续绑定 `b150a551b8d465e31e418e1b2eaf5e79bbb7d28e`（`0.1.1-rc.2`），见 [`docs/compatibility.md`](../compatibility.md)

## 1. 文档目的与效力

本文把产品需求转化为 v0.2 的模块职责、稳定契约、状态模型和验证边界。它是 `0.2.0`–`0.4.0` 核心流程的已发布架构记录，并继续约束未发布的 `0.5.0-alpha.1` 源码候选。文中“切片开始前”、“必须先探针”等措辞记录当时的交付门禁，不表示当前尚未实现。

`0.4.0` 不改动上述产品流程、状态机或存储格式，只替换 DSH `0.1.2-rc.1` 已删除的宿主集成层；`0.5.0-alpha.1` 进一步适配 alpha.2 的 SessionHandle/v2 会话契约，仍不增加产品行为或数据迁移。当前覆盖规则见 [`dsh-compatibility.md`](dsh-compatibility.md)：浏览器请求通过 DSH Remote/API Gateway 和一次性启动令牌换取的认证 Cookie 到达 Host；本文后续出现的 `/run2skill` 私有 loopback RPC 是 `0.3.1` 的历史实现。
...
当时的交付规则是：切片 A/B 不得宣称 Run -> Skill 闭环成功，只有切片 C 通过 Web Human Review 和回读后才可以。`0.3.0` 在该已落地闭环上完善真实 Web 草稿生成、审核/刷新体验和 DSH 兼容性；`0.3.1` 发布后续整理、修订与长证据强化；`0.4.0` 与未发布的 `0.5.0-alpha.1` 只替换 DSH 兼容层。进一步变化以 [CHANGELOG](../../CHANGELOG.md) 为准。
...
下表记录 `0.2.0` 发布前的阻塞条件；这些契约已通过当时的发布验证。未发布 `0.5.0-alpha.1` 对 DSH `0.1.3-alpha.2`、当前稳定版 `0.4.0` 对 DSH `0.1.2-rc.1`、旧版 `0.3.1` 对 DSH `0.1.1-rc.2` 的验证结论以 [`docs/compatibility.md`](../compatibility.md) 为准。
...
其中核心参数由 `0.2.0`–`0.4.0` 的已发布代码与测试冻结，并继续约束未发布 `0.5.0-alpha.1`；`0.3.1` 对 evidence 选择、真实 route envelope、手动整理调度和 Proposal revision 做了本文已明确记录的强化。本文不重复具体 policy constant；未来调整仍不得改变冻结的 provider/scope/review/publication 语义。若实测要求改变产品行为，必须回到 PRD。
...
- 当前适用：原发布与安全边界继续有效；#84 的 SessionBatch 核心流程已于 `0.2.0` 落地，`0.4.0` 与未发布 `0.5.0-alpha.1` 继续沿用。
