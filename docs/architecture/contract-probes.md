...
实际：12 tests 全部通过。四条发布路径都只产生 `RootBindingV2`，没有 `snapshot.roots` 或 root `observationDigest`；PROJECT/USER winner 分别为 `filesystem/project-dsh` 与 `filesystem/user-dsh`，path 和 content 与批准目标一致；四种配置漂移均进入 `NEEDS_ATTENTION` 且目标保持 absent；provider remount 后 USER Skill 仍由 stock filesystem provider 返回。runner 前后确认受保护 DSH source HEAD/status 不变，并输出 `DSH_SOURCE_AFTER=unchanged`、`CP_ROOT_003=PASS`。
