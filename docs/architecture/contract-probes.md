...
步骤：runner 从受保护 source 创建 fresh disposable clone，复制当前 production source 与 probe；production Host 从 exact Agent 的 active stock Loader filesystem 条目解析 provider/default/custom/explicit-home 配置，探针用同一配置对象挂载真实 Storage/Workspace/Skill Registry/filesystem provider 并驱动该解析；分别执行 PROJECT CREATE、PROJECT MERGE、USER CREATE、USER MERGE 的 Proposal build、immutable Approval、C5 CAS/journal、complete snapshot 与 exact `get()`；四种配置漂移都推进 production builder/saga，且分别模拟若继续会观察 `NO_JOURNAL`/`written` 的恢复分支；USER CREATE 后销毁并重新挂载 stock Skill Registry/filesystem，验证普通 Skill 文件仍被原生加载。
实际：12 tests 全部通过。四条发布路径都只产生 `RootBindingV2`，没有 `snapshot.roots` 或 root `observationDigest`；PROJECT/USER winner 分别为 `filesystem/project-dsh` 与 `filesystem/user-dsh`，path 和 content 与批准目标一致；四种真实 provider composition 漂移的 production revalidator 都精确返回 `ROOT_CONTRACT_UNSUPPORTED`，saga 在 journal recover、root prepare、write、finalize 与 readback 调用前进入 `NEEDS_ATTENTION`，目标保持 absent；provider remount 后 USER Skill 仍由 stock filesystem provider 返回。runner 前后确认受保护 DSH source HEAD/status 不变，并输出 `DSH_SOURCE_AFTER=unchanged`、`CP_ROOT_003=PASS`。
...
证据命令：`powershell -File probes/run-dsh-root-contract-probe.ps1 -DshSource <path-to-clean-dsh-checkout>`；固定 baseline 结果为 1 个文件、12 个测试通过，结尾为 `DSH_SOURCE_AFTER=unchanged`、`CP_ROOT_003=PASS`。
